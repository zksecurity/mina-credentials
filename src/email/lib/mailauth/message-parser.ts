...
import type { ParsedHeaders } from './dkim-verifier.ts';
  ...
  async processChunk(chunk: Buffer) {
    ...
    if (this.state === 'header') {
      ...
      for (let i = 0; i < chunk.length; i++) {
        ...
        let b1 =
          this.stateBytes.length > 1 &&
          this.stateBytes[this.stateBytes.length - 2];
        let b2 =
          this.stateBytes.length > 2 &&
          this.stateBytes[this.stateBytes.length - 3];
