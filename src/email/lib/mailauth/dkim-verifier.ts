...
import { MessageParser } from './message-parser.ts';
import { dkimBody } from './body/index.ts';
import { generateCanonicalizedHeader } from './header/index.ts';
...
export class DkimVerifier extends MessageParser {
  ...
  async messageHeaders(headers: ParsedHeaders) {
    ...
    this.signatureHeaders = headers.parsed
      ...
      .map((h) => {
        const value: ParseDkimHeaders & { [key: string]: any } =
          parseDkimHeaders(h.line);
        value.type = 'DKIM';
        return value;
      });
    ...
    if (this.options.sender) {
      ...
      this.envelopeFrom =
        returnPath.length && returnPath[0].address
          ? returnPath[0].address
          : false;
    } else {
      let returnPathHeader = headers.parsed
        .filter((h) => h.key === 'return-path')
        .pop();
      if (returnPathHeader) {
        let returnPathHeaderString = returnPathHeader.line.toString();
        let splitterPos = returnPathHeaderString.indexOf(':');
        if (splitterPos >= 0) {
          returnPathHeaderString = returnPathHeaderString.substr(
            splitterPos + 1
          );
        }
    ...
    for (let signatureHeader of this.signatureHeaders) {
      ...
      signatureHeader.signAlgo = signatureHeader.algorithm
        .split('-')
        .shift()
        .toLowerCase()
        .trim();
      signatureHeader.hashAlgo = signatureHeader.algorithm
        .split('-')
        .pop()
        .toLowerCase()
        .trim();
      ...
      signatureHeader.headerCanon =
        signatureHeader.canonicalization
          .split('/')
          .shift()
          .toLowerCase()
          .trim() || 'simple';
      ...
      signatureHeader.bodyCanon = (
        signatureHeader.canonicalization.split('/')[1] || 'simple'
      )
        .toLowerCase()
        .trim();
      ...
      signatureHeader.maxBodyLength =
        ...
        !isNaN(signatureHeader.parsed?.l?.value)
          ? signatureHeader.parsed?.l?.value
          : '';

      const validSignAlgo = ['rsa', 'ed25519'];
      const validHeaderAlgo =
        signatureHeader.type === 'DKIM' ? ['sha256', 'sha1'] : ['sha256'];
      const validHeaderCanon =
        signatureHeader.type !== 'AS' ? ['relaxed', 'simple'] : ['relaxed'];
      const validBodyCanon =
        signatureHeader.type !== 'AS' ? ['relaxed', 'simple'] : ['relaxed'];
  ...
  async finalChunk() {
    try {
      ...
      for (let signatureHeader of this.signatureHeaders) {
        ...
        let { canonicalizedHeader } = generateCanonicalizedHeader(
          signatureHeader.type,
          signingHeaderLines as any,
          {
            signatureHeaderLine: signatureHeader.original as string,
            canonicalization: signatureHeader.canonicalization,
            instance: ['ARC', 'AS'].includes(signatureHeader.type)
              ? signatureHeader.parsed?.i?.value
              : false,
          }
        );
        ...
        let status: { [key: string]: any } = {
          ...
          header: {
            // signing domain
            i: signatureHeader.signingDomain
              ? `@${signatureHeader.signingDomain}`
              : false,
            // dkim selector
            ...
            // signature value
            b: signatureHeader.parsed?.b?.value
              ? `${signatureHeader.parsed?.b?.value.substr(0, 8)}`
              : false,
          },
        };
        ...
        if (signatureHeader.type === 'DKIM' && this.headerFrom?.length) {
          status.aligned = this.headerFrom?.length
            ? getAlignment(this.headerFrom[0]! ?? ''.split('@')?.pop(), [
                signatureHeader.signingDomain,
              ])
            : false;
        }
        ...
        signatureHeader.bodyHashedBytes = this.bodyHashes.get(
          signatureHeader.bodyHashKey
        )?.bodyHashedBytes;
