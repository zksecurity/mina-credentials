...
export class RelaxedHash {
  ...
  update(chunk: Buffer | null, final: boolean) {
    ...
    if (
      this.remainder &&
      this.remainder instanceof Buffer &&
    ...
    if (chunk && cursorPos < chunk.length && cursorPos !== lineEndPos) {
      ...
      if (chunkPart.length) {
        this.pushBodyHash(
          lineNeedsFixing ? this.fixLineBuffer(chunkPart) : chunkPart
        );
        lineNeedsFixing = false;
      }
    ...
    if (final) {
      let chunkPart =
        (cursorPos && chunk && chunk.subarray(cursorPos)) || chunk;
