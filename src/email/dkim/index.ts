...
const { pki } = pkg;
import { DkimVerifier } from '../lib/mailauth/dkim-verifier.ts';
import { writeToStream } from '../lib/mailauth/tools.ts';
import sanitizers from './sanitizers.ts';
import { resolveDNSHTTP } from './dns-over-http.ts';
import { resolveDNSFromZKEmailArchive } from './dns-archive.ts';
...
export async function verifyDKIMSignature(
  ...
  let dkimResult = await tryVerifyDKIM(
    email,
    domain,
